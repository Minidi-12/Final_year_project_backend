...
const detectIntent = (text: string): string => {
  ...
  if (t.match(/\b(status|check|request|my request|where|update)\b/))
    return "CHECK_STATUS";
  ...
  if (t.match(/^[0-9a-f]{24}$/i)) return "IS_REF";
...
export const handleIncomingMessage = async (from: string, body: string) => {
  ...
  if (intent === "CANCEL") {
    ...
    return sendWhatsApp(
      phone,
      " Session ended. Text *hi* anytime to start again.\n\n_HopeConnect Foundation_",
    );
  }
  ...
  if (session.step === "MENU" || intent === "GREETING") {
    if (text === "1" || intent === "CHECK_STATUS") {
      ...
      return sendWhatsApp(
        phone,
        ` *Request Status Check*\n\nPlease send your:\n *NIC Number* (e.g. 199012345678)\n— or —\n *Reference ID* (e.g. 6a15f02afa1d9c3702ab7d82)`,
      );
    }
    if (text === "2") {
      return sendWhatsApp(
  ...
  if (session.step === "AWAIT_NIC_OR_REF") {
    if (intent === "IS_NIC") {
      const req = await B_Req.findOne({ "b_profile.nic": text }).sort({
        created_at: -1,
      });
      clearSession(from);
    ...
    return sendWhatsApp(
      ...
      ` *Invalid format*\n\nPlease send:\n` +
        ...
        `• Reference: 24-character ID (e.g. 6a15f02afa1d9c3702ab7d82)\n\nOr type *cancel* to go back.`,
    );
  }
...
const buildStatusMessage = (req: any): string => {
  ...
  const urgency =
    urgencyEmoji[req.urgency_label?.toLowerCase()] ||
    req.urgency_label ||
    "Not assessed";
...
const getStatusAdvice = (status: string): string => {
  const advice: Record<string, string> = {
    pending:
      " Your request is in the queue. A GN Officer will be assigned soon.",
    gn_assigned:
      " A GN Officer has been assigned and will contact you for verification.",
    verified:
      " Your request has been verified. The NGO is reviewing your case.",
    flagged:
      " Your case needs additional information. An officer will contact you.",
    resolved: " Great news! Support has been arranged for your family.",
    rejected:
      " Your request was not approved. Contact your GN Officer for details.",
  };
